...
public class RoadMeshGeneration
{
    ...
    public Mesh GenerateMesh(Vector3[] points, float[] wayWidths, float vertexSpacing)
    {
        ...
        Vector3Poly[] centralPPolynom = PathPiecewisePolynom(points);

        for (int i = 0; i < wayDividers.Count; i++)
        {
            Vector3 mod = Vector3.Cross(centralPPolynom[0].CalculateFirstDerivative(centralPPolynom[0].getFirstKnot()), Vector3.up);
            mod.y = 0;
            wayDividers[i].addPoint(points[0] + mod.normalized * -wayDividers[i].getSidewaysShift());
        }

        for (int i = 1; i < points.Length; i++)
        {
            for (int j = 0; j < wayDividers.Count; j++)
            {
                Vector3 mod = Vector3.Cross(centralPPolynom[i - 1].CalculateFirstDerivative(centralPPolynom[i - 1].getLastKnot()), Vector3.up);
                mod.y = 0;
                wayDividers[j].addPoint(points[i] + mod.normalized * -wayDividers[j].getSidewaysShift());
            }
        }
